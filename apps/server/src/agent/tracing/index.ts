...
import { LangSmithTracer, type TraceExportTelemetry } from "./langsmith-tracer.js";
...
export function createAgentTracer(config: ServerConfig, telemetry: TraceExportTelemetry = {}): AgentTracer {
  ...
  return new LangSmithTracer(config, telemetry);
}

export function createTraceRegistry(config: ServerConfig, telemetry: TraceExportTelemetry = {}): TraceRegistry {
  return new TraceRegistry(createAgentTracer(config, telemetry));
}
